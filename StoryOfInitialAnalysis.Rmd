...
For the following heatmap I took the significant genes for the 2hpf and 8hpf control experiments and hierarchically clustered all of the timepoints for each reference control together. 
...
Heatmap B shows the clusters that are created from clustering purely from the zscore of the mean RPKM of significant genes from both the 8hr control and the 2hr reference experiments.

I think this is a pretty good proof a principle that the scaled and centered RPKMs are able to recapitulate similar clusters that contain the same gene lists to the clusters created from the differential expression data for the significant genes, with some minor differences in the order of the genes.
...
pheatmap(rpkm_sig_heat, show_rownames = FALSE, cluster_cols = FALSE, cluster_rows = hr, main = paste0('Heatmap B: Clustering of the zscore of the RPKM of ', nrow(rpkm_sig), ' Genes'), border_color = NA, annotation_row=annotation_rpkm, annotation_colors = ann.cols, color = colorRampPalette(brewer.pal(n = 7, name = "OrRd"))(100))
...
Here I did a PCA analysis on the mean RPKM per sample for the significant genes in the heatmaps above. We see that PC2 holds the variance that separates the 'early' transcripts from the 'late' transcripts. PC1 did not provide differentiation between the timepoints and may reflect technical variation.
...
The following is a plot of the trends for each of the clusters in Heatmap A, with the 95% confidence interval on the error bars for both the mean z-score and the mean RPKM.
...
d5 <- datatable(rpkm_sub, rownames = FALSE, extensions = c('Buttons','Scroller', 'ColReorder'), options = list(
    ...
    scrollY = 800,
    colReorder = TRUE,
    scroller = TRUE), caption = 'Table 1: All Genes in associated clusters')
...
The following heatmap shows all significantly differentially expressed genes that are annotated to GO term 'GO0006355': regulation of transcription, DNA-templated.
...
tflfc <- inner_join(tf_factor, lfc) %>% dplyr::select(-sp_id, -sp_desc, -sp_evalue, -go_all, -Pfam)
...
pheatmap(tfl_heat[, keep_cols],  cluster_cols = FALSE, clustering_method = 'complete', clustering_distance_rows = dist(tfl), 
           main = paste0('Clustering of ', nrow(tfl), ' Genes'), border_color = NA, cellwidth = 15, cellheight = 10, 
           annotation_row = annotation_sub, gaps_col = c(4),
           labels_col = c("1hpf", "2hpf", "3hpf", "4hpf","5hpf", "6hpf", "7hpf", "8hpf"), 
           annotation_colors = ann.colors, annotation_col = annot_column, annotation_names_col = FALSE,
...
The following are heatmaps for differentially expressed genes annotated to the following GO terms:


<font color="DarkGray"> Table 2: Descriptor GO terms with link to the Gene Ontology page associated.</font>


GO ID | Description | Gene Ontology Link |
------|--------------------------------------|-----------|
GO0007049    |       Cell Cycle|http://amigo.geneontology.org/amigo/term/GO:0007049
GO0048729 |Tissue Morphogenesis|http://amigo.geneontology.org/amigo/term/GO:0048729
GO0030154 |Cell Differentiation|http://amigo.geneontology.org/amigo/term/GO:0030154
GO0007276  |  Gamete Generation|http://amigo.geneontology.org/amigo/term/GO:0007276
GO0007498 |Mesoderm Development|http://amigo.geneontology.org/amigo/term/GO:0007498
GO0007492 |Endoderm Development|http://amigo.geneontology.org/amigo/term/GO:0007492
GO0007398 |Ectoderm Development|http://amigo.geneontology.org/amigo/term/GO:0007398
...
for(x in heatmaps){
  ...
  if(nrow(df) > 25){
      ...
      pheatmap(df[, keep_cols],  cluster_cols = FALSE, clustering_method = 'complete', clustering_distance_rows = dist(df_new), 
               main = x[[3]], border_color = NA, cellwidth = 15, cellheight = 10, 
               annotation_row = annotation_sub, gaps_col = c(4),
               labels_col = c("1hpf", "2hpf", "3hpf", "4hpf","5hpf", "6hpf", "7hpf", "8hpf"),
               annotation_colors = ann.colors, annotation_col = annot_column, annotation_names_col = FALSE, filename = filename)
      cat(paste0("#####[Link to ", x[[1]]," Heatmap](http://bioinfo/home/klg/projects/Mnemiopsis/website/", filename, ")<br><br><br>"))
    } else {
  pheatmap(df[, keep_cols],  cluster_cols = FALSE, clustering_method = 'complete', clustering_distance_rows = dist(df_new), 
           main = x[[3]], border_color = NA, cellwidth = 15, cellheight = 10, 
           annotation_row = annotation_sub, gaps_col = c(4),
...
### Bar Plot
The following bar plot shows the number of significantly differenctially expressed genes annotated to each of the GO terms in Table 2.
...
DT::datatable(dplyr::select(dea_go1, ID, Term),
               caption = 'Table 3: Mnemiopsis Genes Annotated to the GO terms in Table 2.')
...
The following figure shows the significantly enriched GO terms that had at least 5 genes annotated, and a Benjamini-Hochberg adjusted p-value less than or equal to 0.05 for both the Early and Late stage genes. 
...
DT::datatable(dplyr::select(table, go_id, Term, Ontol, EnrichID, BH.correction, Num_Genes))
