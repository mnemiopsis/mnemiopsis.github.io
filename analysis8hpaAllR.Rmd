---
title: "8hpa Control - All Replicates" 
...
```{r initializeData8hpaControlAllReplicates, echo = FALSE, results = "hide", message = FALSE, error = FALSE, warning = FALSE, cache = FALSE}

## user input to 'DESeq2All.Rdata'

load("./output/DESeq2AllAllR.Rdata")
setwd("./")
...
# side <- function(text) {
#   div(class = "sidenote-right", p(text))
# }
# 
# text <- "Spearman's rank correlation coefficient is a nonparametric measure of rank correlation, recommended for data that doesn't necessarily come from a bivariate normal distribution."
# 
# strText <- strsplit(text, "")[[1]]
# 
# char = 0 
# for(i in strText){
#   string = ''
#   if(i in c(LETTERS, tolower(LETTERS), ".", "'")){
#     char = char + 1
...
#   
# ```{r, results = 'asis'}
# side("Spearman's rank correlation")
# ```
...
The following plots the spearman correlation of the RPM (reads per million) for all genes that have an average RPM &#8805; 1.

Spearman's rank correlation coefficient is a nonparametric measure of rank correlation, recommended for data that doesn't necessarily come from a bivariate normal distribution.


```{r spearman_correlation8hpaControlAllReplicates, fig.cap = ""}
...
```{r PCA8hpaControlAllReplicates, fig.fullwidth = TRUE, fig.width = 10, fig.height = 10}
...
```{r mavol8hpaControlAllReplicates}
...
```{r plotmavol8hpaControlAllReplicates, fig.keep = "all", fig.height = 10, fig.width = 20, echo=FALSE, results='asis'}
...
```{r pairwiseHeatmaps8hpaControlAllReplicates, results = "hide", fig.cap = "", fig.keep = "high"}
...
```{r session8hpaControlAllReplicates, echo=FALSE}
writeLines(capture.output(sessionInfo()), "sessionInfo/8hpaAllSessionInfo.txt")
```
